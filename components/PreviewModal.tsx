...
import { ProcessedFile, ProcessedResult } from '../types';
...
const PreviewModal: React.FC<PreviewModalProps> = ({
  ...
  const currentUrl = isOriginal ? file.previewUrl : (typeof activeTab === 'number' ? blobUrls.get(activeTab) || '' : '');
  const currentSize = isOriginal ? file.originalSize : (currentResult?.size || 0);
  const currentFormat = isOriginal ? 'Original' : currentResult?.format.split('/')[1].toUpperCase().replace('SVG+XML', 'SVG') || 'IMG';
        ...
        <div className="flex flex-col border-b border-(--border) bg-(--bg-card)">
          ...
          <div className="flex items-center justify-between p-4">
            <div className="flex flex-col min-w-0">
              <h3 className="text-lg font-semibold truncate max-w-md text-(--text-main)" title={fileName}>{fileName}</h3>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex items-center gap-2 px-4 pb-0 overflow-x-auto">
            <button
              ...
              className={`relative px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap border-b-2 ${isOriginal
            ...
            {file.results.map((res, idx) => {
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap border-b-2 ${isActive
                    ? 'border-(--primary) text-(--primary)'
                    : 'border-transparent text-(--text-muted) hover:text-(--text-main)'
                    }`}
                >
        ...
        <div className="px-4 py-2 flex items-center justify-between text-xs bg-(--bg-subtle) border-b border-(--border)">
          <div className="flex items-center gap-3">
            <span className="font-mono text-(--text-muted)">
              {isOriginal ? `${file.originalWidth}×${file.originalHeight}` : `${currentResult?.width}×${currentResult?.height}`}
            </span>
            {!isOriginal && currentResult && (
              <span className={`px-1.5 py-0.5 rounded font-bold ${currentResult.size < file.originalSize
                ? 'bg-(--success) text-white'
                : 'bg-(--error) text-white'
                }`}>
                {currentResult.size < file.originalSize ? 'Saved ' : 'Increased '}
                {Math.abs(Math.round(((file.originalSize - currentResult.size) / file.originalSize) * 100))}%
              </span>
            )}
          </div>
